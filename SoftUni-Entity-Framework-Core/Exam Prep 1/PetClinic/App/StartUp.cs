...
//				BonusTask(context);
        ...
        private static void ImportEntities(PetClinicContext context, string baseDir = @"../Datasets/")
        {
            ...
            var animalAids =
                DataProcessor.Deserializer.ImportAnimalAids(context, File.ReadAllText(baseDir + "animalAids.json"));
            PrintAndExportEntityToFile(animalAids, exportDir + "AnimalAidsImport.txt");

            var animals =
                DataProcessor.Deserializer.ImportAnimals(context, File.ReadAllText(baseDir + "animals.json"));
            PrintAndExportEntityToFile(animals, exportDir + "AnimalsImport.txt");

            var vets = DataProcessor.Deserializer.ImportVets(context, File.ReadAllText(baseDir + "vets.xml"));
            PrintAndExportEntityToFile(vets, exportDir + "VetsImport.txt");

            var procedures =
                DataProcessor.Deserializer.ImportProcedures(context, File.ReadAllText(baseDir + "procedures.xml"));
        ...
        private static void ExportEntities(PetClinicContext context)
        {
            ...
            var animalsExport = DataProcessor.Serializer.ExportAnimalsByOwnerPhoneNumber(context, "0887446123");
            PrintAndExportEntityToFile(animalsExport, exportDir + "AnimalsExport.json");

            var proceduresExport = DataProcessor.Serializer.ExportAllProcedures(context);
